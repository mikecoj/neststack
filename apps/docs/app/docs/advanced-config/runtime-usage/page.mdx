...
import { ConfigStore, CONFIG_STORE } from '@nestx/advanced-config';
...
Feature modules access the global `ConfigService` without importing `AdvancedConfigModule` (when `isGlobal: true`):
