...
`@nestx/advanced-config` is designed to be easy to test. The key pattern: use `envSource` to inject fake environment variables and `AdvancedConfigModule.reset()` to clear state between tests.
...
import { AdvancedConfigModule, ConfigService, defineConfig } from '@nestx/advanced-config';
import { z } from 'zod';
```
...
it('should register feature config', async () => {
  ...
  const module = await Test.createTestingModule({
    imports: [
      AdvancedConfigModule.forRoot({ configs: [] }),
...
The `@nestx/advanced-config` package enforces **100% code coverage** on lines, branches, functions, and statements. When adding new features, ensure all code paths are tested:
