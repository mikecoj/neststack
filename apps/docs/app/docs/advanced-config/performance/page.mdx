...
`@nestx/advanced-config` is optimized for read-heavy workloads. Configuration is read thousands of times during an application's lifetime but written only once at startup.
